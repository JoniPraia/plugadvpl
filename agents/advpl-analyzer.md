...
2. **Arch primeiro** — sempre. Roda `uvx plugadvpl@0.3.1 arch <arquivo>` para overview (source_type, capabilities, funções, tabelas, includes). Isso dá o esqueleto.

3. **Callers/callees do ponto de entrada.** Para a função principal (ou a indicada), rode em paralelo:
   - `uvx plugadvpl@0.3.1 callers <funcao>` — quem chama (entendido o "porquê").
   - `uvx plugadvpl@0.3.1 callees <funcao>` — o que ela chama (entendido o "como").
...
5. **Cruzamentos extras quando útil:**
   - `uvx plugadvpl@0.3.1 grep "<termo>"` — busca regex no projeto (ex.: outras chamadas de uma constante).
   - `uvx plugadvpl@0.3.1 tables --reclock` — confirma transações de gravação.
   - `uvx plugadvpl@0.3.1 param <SX6>` — explica parâmetros mencionados.
...
- `uvx plugadvpl@0.3.1 callers <fn>` — quem chama.
- `uvx plugadvpl@0.3.1 callees <fn>` — o que chama.
- `uvx plugadvpl@0.3.1 grep "<re>"` — busca regex ranqueada.
- `uvx plugadvpl@0.3.1 find function 'MA010*'` — descobre funções por glob.
- `uvx plugadvpl@0.3.1 tables --read|--write|--reclock` — tabelas tocadas.
- `uvx plugadvpl@0.3.1 param <SX6>` — parâmetros referenciados.
