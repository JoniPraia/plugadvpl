...
2. **Workflow função:**
   - `uvx plugadvpl@0.1.0 callers <fn>` — todos os pontos de chamada.
   - `uvx plugadvpl@0.1.0 callees <fn>` — o que ela chama (entender efeito cascata).
   - Para cada caller, registrar: arquivo, linha, contexto (loop? transação? PE?).

3. **Workflow tabela:**
   - `uvx plugadvpl@0.1.0 tables --read <ALIAS>` — quem lê.
   - `uvx plugadvpl@0.1.0 tables --write <ALIAS>` — quem grava (DELETE/UPDATE).
   - `uvx plugadvpl@0.1.0 tables --reclock <ALIAS>` — quem trava (RecLock).

4. **Workflow campo:**
   - `tables --read/--write/--reclock` no alias.
   - `uvx plugadvpl@0.1.0 grep "<ALIAS>->\\?<CAMPO>"` para pegar uso explícito.
   - Cruze com `arch` dos arquivos resultantes para entender capabilities.

5. **Workflow parâmetro:**
   - `uvx plugadvpl@0.1.0 param <NOME>` — definição + descrições.
   - `uvx plugadvpl@0.1.0 grep "GetMv\\(\\s*['\\\"]<NOME>"` — usos por GetMv/GetNewPar.
...
- `uvx plugadvpl@0.1.0 callers <fn>`
- `uvx plugadvpl@0.1.0 callees <fn>`
- `uvx plugadvpl@0.1.0 tables --read|--write|--reclock [ALIAS]`
- `uvx plugadvpl@0.1.0 param <PARAM>`
- `uvx plugadvpl@0.1.0 grep "<regex>"`
- `uvx plugadvpl@0.1.0 arch <arq>` — para enriquecer contexto de cada caller.
